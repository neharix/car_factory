...
class CarForm(ModelForm):
    users = forms.MultipleChoiceField(choices=choices, widget=forms.SelectMultiple(attrs={"class": "form-control"}))
    class Meta:
        model = Car
        fields = ('car_name', 'car_desc', 'image', 'users', 'vehicle_type', 'color', 'car_year', 'vehicle_number')
        widgets = {
...
class UserForm(ModelForm):
    class Meta:
        model = User
        fields = ('first_name', 'last_name', 'father_name', 'username', 'password', 'email', 'documents', 'passport_serie')
        widgets = {
